...
HEADERS = {
    ...
    'Authorization': f"Bearer {os.getenv('AHREFS_API_KEY')}"
}

# Fetch the list of competitors from BigQuery
def get_domains_from_bq():
    client = bigquery.Client()
    input_table = "couplerio-demo.ahrefs_dashboard.backlinks_monitor"
...
def fetch_ahrefs_data():
    ...
    if backlinks_results:
        export_to_bigquery(pd.DataFrame(backlinks_results), "couplerio-demo.ahrefs_dashboard.new_backlinks")
    if refdomains_results:
        export_to_bigquery(pd.DataFrame(refdomains_results), "couplerio-demo.ahrefs_dashboard.new_backlinks_refdomains")
